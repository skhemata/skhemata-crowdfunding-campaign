...
export class campaignBackers extends SkhemataBase {
  ...
  render() {
    return html`
      <div class="columns">
        <div class="backers-container column">
          ${this.campaignBackers}
        </div>
      </div>
    `;
  }
  ...
  private getBackers() {
    fetch(
      `${this.apiUrl}${this.locPath}campaign/${this.campaignId}/backer`
    )
    .then(response => {
      if(!response.ok){
        return;
    ...
    if(this.backers) {
      ...
      for (let key in this.backers) {
        this.campaignBackers = html`<div class='faq-title'>
          ...
          ${this.backers[key].total_amount}
          Backed ${this.backers[key].total_backed} ${(this.backers[key].total_backed === 1) ? "Campaign" : "Campaigns"}
        </div>
        `;
      }
    }
